...
interface SkipToPhase2Props {
  onConfirm: () => void;
  onCancel: () => void;
}

export const SkipToPhase2: React.FC<SkipToPhase2Props> = ({ onConfirm, onCancel }) => {
  return (
    <motion.div 
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
    ...
    >
      ...
      >
        ...
        <div className="relative z-10">
          ...
          <h2 className="text-3xl font-black uppercase tracking-tighter mb-4 leading-none">Skip to Phase 2?</h2>
          <p className="text-muted-grey text-lg font-medium mb-8 leading-relaxed">
            This will unlock all Phase 1 evidence and move the investigation to the next stage. Use this only for debugging or if you're stuck.
          </p>
          
          <div className="flex flex-col gap-3">
            <button 
              onClick={onConfirm}
              className="w-full py-5 bg-amber-500 text-black font-black rounded-2xl hover:bg-amber-400 transition-all shadow-xl shadow-amber-500/20 flex items-center justify-center gap-3 uppercase tracking-widest text-sm"
            >
              SKIP TO PHASE 2 <ArrowRight size={20} />
            </button>
            <button 
              onClick={onCancel}
